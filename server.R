server <- function(input, output, session) {
  ...
  output$revenue_start_stage_selector <- renderUI({
    df <- DF()
    choices <- setNames(1:nrow(df), paste("After Stage", 1:nrow(df), "-", df$`Stage Name`))
    selectInput("revenue_start_stage", "Revenue begins after stage:", 
                choices = choices, selected = nrow(df))
  })
  
  # Real-time milestone share warning
  output$milestone_share_warning <- renderUI({
    # Only check when milestones+AMC is selected
    if (is.null(input$mechanisms) || input$mechanisms != "milestones_amc") {
      return(NULL)
    }
    ...
    if (nrow(zero_share_stages) > 0) {
      stage_names <- paste(
        sprintf("Stage %d (%s)", zero_share_stages$Stage, zero_share_stages$`Stage Name`),
        collapse = ", "
      )
      errors <- c(errors, list(
        sprintf("Milestone selected but Stage Share is 0%% for: %s. Please assign a percentage or uncheck the milestone.", 
                stage_names)
      ))
    }
    ...
    if (total_pct > 100) {
      errors <- c(errors, list(
        sprintf("Milestone percentages sum to %.1f%%, which exceeds 100%%. Please reduce the Stage Share values.", 
                total_pct)
      ))
    }
      ...
      error_ui <- div(
        ...
        if (length(errors) == 1) {
          p(style = "margin: 0; color: #991b1b; font-size: 13px; font-weight: 600; line-height: 1.5;",
            HTML(paste0("⚠️ ", errors[[1]])))
        } else {
          tagList(
            p(style = "margin: 0 0 8px 0; color: #991b1b; font-size: 13px; font-weight: 600;",
              "⚠️ Milestone configuration issues:"),
            tags$ul(
              style = "margin: 0; padding-left: 20px; color: #7f1d1d; font-size: 12px;",
    ...
    if (length(warnings) > 0 && length(errors) == 0) {
      warning_ui <- div(
        style = "margin-top: 10px; margin-bottom: 10px; padding: 12px 16px; 
               ...
               border-radius: 8px;",
        p(style = "margin: 0; color: #92400e; font-size: 13px; font-weight: 500; line-height: 1.5;",
          HTML(paste0("ℹ️ ", warnings[[1]])))
      )
    }
  ...
  output$stage_data_warning <- renderUI({
    ...
    if ("Stage Name" %in% names(df)) {
      missing_names <- which(is.na(df$`Stage Name`) | trimws(df$`Stage Name`) == "")
    ...
    if ("Probability (%)" %in% names(df)) {
      over_rows <- which(!is.na(df$`Probability (%)`) & df$`Probability (%)` > 100)
    ...
    if (length(missing_info) > 0) {
      for (col in names(missing_info)) {
        ...
        warnings <- c(warnings, list(
          sprintf("Missing %s in stage%s %s", 
                  col,
                  if(length(rows) > 1) "s" else "",
                  paste(rows, collapse = ", "))
        ))
      }
    }
    ...
    if (length(negative_info) > 0) {
      for (col in names(negative_info)) {
        ...
        warnings <- c(warnings, list(
          sprintf("Negative %s in stage%s %s (must be ≥ 0)", 
                  col,
                  if(length(rows) > 1) "s" else "",
                  paste(rows, collapse = ", "))
        ))
    ...
    if (!is.null(prob_over_100)) {
      warnings <- c(warnings, list(
        sprintf("Probability exceeds 100%% in stage%s %s",
                if(length(prob_over_100) > 1) "s" else "",
  ...
  validation_status <- reactive({
    ...
    df <- hot_to_r(input$stages_table)
    if (!is.null(df) && nrow(df) > 0) {
      required_cols <- c("Duration", "Cost", "Probability (%)", "Share of costs covered by other sources (%)")
      
      for (col_name in required_cols) {
        if (col_name %in% names(df)) {
          if (any(is.na(df[[col_name]]))) {
            errors <- c(errors, list(sprintf("Missing values in '%s' column. Please fill in all required fields.", col_name)))
      ...
      if (any(!is.na(df$Duration) & df$Duration < 0) ||
          ...
          any(!is.na(df$`Probability (%)`) & df$`Probability (%)` < 0)) {
        errors <- c(errors, list("Stage data contains negative values. Duration, Cost, and Probability must be ≥ 0."))
      }
      
      # Check probability bounds
      if (any(!is.na(df$`Probability (%)`) & df$`Probability (%)` > 100)) {
    ...
    if (!is.null(input$mechanisms) && input$mechanisms == "milestones_amc") {
      ...
      if (!is.null(df) && "Milestone?" %in% names(df)) {
        ...
        # if (nrow(milestone_stages) == 0) {
        #   errors <- c(errors, list("Milestones + AMC requires at least one stage marked as a milestone. Please check the 'Milestone?' box for at least one stage."))
        # }
        ...
        if (nrow(milestone_stages) > 0) {
          total_pct <- sum(milestone_stages$`Stage Share (%)`, na.rm = TRUE)
          if (total_pct > 100) {
            errors <- c(errors, list(sprintf("Milestone percentages sum to %.1f%%, which exceeds 100%%. Please adjust.", total_pct)))
          }
          
          # ERROR: Milestone with zero stage share  <-- ADD THIS BLOCK
    ...
    if (!is.null(input$use_feasibility) && input$use_feasibility) {
      if (!is.null(input$target_probability) && !is.null(input$eta)) {
        if (input$target_probability >= input$eta) {
          errors <- c(errors, list(
            sprintf("Target probability (%.0f%%) cannot exceed or equal global possibility parameter (%.0f%%). Please adjust your parameters.",
                    input$target_probability , input$eta )
          ))
        }
      }
    }
    ...
    if (is.null(input$mechanisms) || length(input$mechanisms) == 0) {
      warnings <- c(warnings, list("Please select an incentive mechanism before calculating."))
    }
    ...
    if (!is.null(input$amc_calculation_mode) && 
        ...
        (input$mechanisms == "amc" || input$mechanisms == "milestones_amc")) {
      
      feasibility <- amc_feasibility_check()
      if (!is.null(feasibility) && !feasibility$feasible) {
        errors <- c(errors, list(
          sprintf("AMC top-up insufficient. Minimum needed: $%s/unit. Please increase the top-up amount, lifecycle duration, or monthly units.",
                  format(round(feasibility$min_topup, 2), big.mark = ",", scientific = FALSE))
        ))
      }
    }
    ...
    for (inp in numeric_inputs) {
      if (!is.null(inp$val) && (is.na(inp$val) || !is.numeric(inp$val))) {
        errors <- c(errors, list(sprintf("%s must be a valid number", inp$name)))
      }
    }
    ...
    list(errors = errors, warnings = warnings, has_errors = length(errors) > 0)
  })
    ...
    tagList(
      ...
      if (length(val$errors) > 0) {
        lapply(val$errors, function(err) {
          div(class = "warning-box", style = "background: linear-gradient(135deg, rgba(239, 68, 68, 0.15), rgba(220, 38, 38, 0.1)); border-left: 4px solid #ef4444;",
              p(class = "warning-text", style = "color: #991b1b; margin: 0;",
                tags$strong("⚠️ Error: "), err)
          )
        })
      },
      ...
      if (length(val$warnings) > 0) {
        lapply(val$warnings, function(warn) {
          div(class = "warning-box", style = "background: linear-gradient(135deg, rgba(251, 191, 36, 0.15), rgba(245, 158, 11, 0.1)); border-left: 4px solid #f59e0b;",
              p(class = "warning-text", style = "color: #92400e; margin: 0;",
                tags$strong("⚠️ Warning: "), warn)
          )
        })
      }
    )
    ...
    if (saved_archetypes$push_funding_visible) {
      ...
      div(
        ...
        p(style = "margin: 0; font-size: 15px; color: #2d3748; font-weight: 500;",
          sprintf("If grants or other push funding were provided to each firm at every stage, the total cost would be $%s%s. There is a %.1f%% chance that none of the %d firms would succeed, meaning all payments would be wasted.",
  ...
  adoption_curve <- reactive({
    ...
    if (!is.null(input$model_revenue) && input$model_revenue) {
      T <- ifelse(is.null(input$revenue_lifecycle), 10, input$revenue_lifecycle)
      uptake_model <- ifelse(is.null(input$revenue_uptake_model), "linear", input$revenue_uptake_model)
    } else {
      T <- ifelse(is.null(input$amc_lifecycle), 10, input$amc_lifecycle)
      uptake_model <- ifelse(is.null(input$amc_uptake_model), "linear", input$amc_uptake_model)
    }
    ...
    if (uptake_model == "linear") {
      ...
      if (!is.null(input$model_revenue) && input$model_revenue) {
        units <- rep(ifelse(is.null(input$revenue_monthly_units), 0, input$revenue_monthly_units), total_months)
      } else {
        units <- rep(ifelse(is.null(input$amc_monthly_units), 0, input$amc_monthly_units), total_months)
      }
    } else {
      # logistic S-shaped
      if (!is.null(input$model_revenue) && input$model_revenue) {
        K <- ifelse(is.null(input$revenue_max_units), 0, input$revenue_max_units)
        r <- ifelse(is.null(input$revenue_growth), 1, input$revenue_growth)
        t0 <- ifelse(is.null(input$revenue_peak_time), total_months/2, input$revenue_peak_time * 12)
      } else {
        K <- ifelse(is.null(input$amc_max_units), 0, input$amc_max_units)
        r <- ifelse(is.null(input$amc_growth), 1, input$amc_growth)
        t0 <- ifelse(is.null(input$amc_peak_time), total_months/2, input$amc_peak_time * 12)
      }
  ...
  validate_milestone_percentages <- function(df) {
    ...
    if (nrow(milestone_stages) == 0) {
      return(list(valid = FALSE, total = 0, error = "No milestone stages selected"))
    }
    ...
    list(
      ...
      error = if (total_pct <= 0) {
        ...
        sprintf("Milestone percentages sum to %.1f%%, cannot exceed 100%%", total_pct)
      } else NULL
    )
  }
  ...
  calculate_to_stage <- function(df, stage_num, discount_rate) {
    ...
    calc_df <- df_to_stage %>%
      ...
      mutate(cost_after_push = cost*(1-push_funding),
  ...
  calculate_milestones <- function(df, discount_rate, target_probability, 
                                   use_feasibility = FALSE, eta = 1,
                                   topup = NULL, ac_template = NULL, unit_factor = NULL,
                                   revenue_pv = 0, global_pos = NULL,
                                   calculation_mode = "topup_to_units",  # ADD THIS
                                   target_units = NULL,
    ...
    full_metrics <- calculate_project_metrics(df, discount_rate, target_probability,
                                              use_feasibility, eta)
    ...
    total_milestone_payments <- sum(sapply(milestone_list, function(m) m$milestone_payment))
    ...
    if (!is.null(ac_template) && nrow(ac_template) > 0) {
      ...
      if (calculation_mode == "units_to_topup" && !is.null(target_units)) {
        ...
        topup_result <- calculate_topup_from_units(
          ...
          target_amount = amc_target_amount,  # This is the AMC portion (x - y)
      ...
      } else if (calculation_mode == "topup_to_units" && !is.null(topup) && topup > 0) {
        ...
        amc_calc <- calculate_amc_for_target(
          ...
          target_units = NULL,
          revenue_start_month = if (!is.null(revenue_start_month)) revenue_start_month else total_duration * 12
        )
    ...
    # Total expected cost
    if (!is.null(amc_component$amc_covers_target) && !is.na(amc_component$amc_covers_target)) {
    ...
    return(list(
      ...
      gross_incentive = gross_incentive,      # Added: track gross for display
      net_incentive = net_incentive,          # Added: track net for display
      n_firms = n_firms,
      global_pos = global_pos,
  ...
  calculated_results <- eventReactive(input$calculate, {
    ...
    if (is.null(mech) || length(mech) == 0) {
      ...
      df_prev <- DF()
      return(calculate_project_metrics(df_prev, input$discount_rate, input$target_probability, input$use_feasibility, if(input$use_feasibility) input$eta/100 else 1))
    }
    ...
    results <- calculate_project_metrics(df, input$discount_rate, input$target_probability, use_feasibility, eta_prop)
    ...
    unit_factor <- switch(isolate(input$cost_unit), "M" = 1e6, "K" = 1e3, "B" = 1e9, "1" = 1)
    ...
    if (!is.null(input$model_revenue) && input$model_revenue) {
      ...
      if (revenue_start_stage == 0) {
        ...
        ac_pv <- ac %>%
          mutate(
            ...
            discount_factor = 1 / ((1 + input$discount_rate/100)^(time_from_present/12)),
        ...
        if (nrow(remaining_stages) > 0) {
          ...
          for (i in 1:nrow(remaining_stages)) {
            ...
            prob_to_this_stage <- if (i == 1) {
              ...
              prob_to_revenue_start * prod(remaining_stages$`Success Probability`[1:(i-1)])
            }
            
            # Mark months in this stage
            ac$phase_prob[ac$month > phase_start & ac$month <= phase_end] <- prob_to_this_stage
      ...
      results$commercially_viable <- FALSE  # ADD THIS LINE
    }
    ...
    if (!is.null(input$model_revenue) && input$model_revenue && revenue_pv > 0) {
    ...
    if (mech == "shared") {
      ...
      if (prob_of_success < 1) {
        if (use_feasibility) {
          n_firms_temp <- ceiling(log(1 - (input$target_probability/100 / eta_prop)) / log(1 - prob_of_success))
        } else {
      ...
      topup_param <- if (calc_mode == "topup_to_units") input$amc_topup else NULL
      units_param <- if (calc_mode == "units_to_topup") input$amc_target_units else NULL
      ...
      global_pos <- if (use_feasibility) pmin(base_global_pos * eta_prop, eta_prop) else base_global_pos
      
      # Calculate AMC needed to cover target
      amc_results <- calculate_amc_for_target(
        target_amount = target_prize,
        topup = topup_param,  # This is correct - uses the conditional value
        ac_template = ac_template,
      ...
      milestone_results <- calculate_milestones(
        ...
        ac_template = adoption_curve(),
        unit_factor = unit_factor,
        revenue_pv = revenue_pv,  # Use the revenue_pv calculated earlier in this function
        global_pos = NULL,  # Let calculate_milestones compute it internally
        calculation_mode = calc_mode,      
        target_units = units_param,
        revenue_start_month = amc_start_month
      )
  ...
  amc_feasibility_check <- reactive({
    ...
    if (is.null(input$amc_calculation_mode) || input$amc_calculation_mode != "topup_to_units") {
      return(NULL)
    }
    ...
    cost_to_attempt <- calculate_project_metrics(df, input$discount_rate, 
                                                 input$target_probability, 
                                                 input$use_feasibility, 
                                                 if(input$use_feasibility) input$eta/100 else 1)$cost_to_attempt
    
    # Calculate revenue if enabled AND determine revenue_start_month
    revenue_pv <- 0
    revenue_start_month <- 0  # ADD THIS - default value
    ...
    if (!is.null(input$model_revenue) && input$model_revenue) {
      ...
      ac_pv <- ac %>%
        mutate(
          profit = units * profit_per_unit,
          time_from_present = revenue_start_month + month,
          discount_factor = 1 / ((1 + discount_rate)^(time_from_present/12)),
          pv_profit = profit * discount_factor * prob_reaching_revenue
        )
      
      unit_factor <- switch(input$cost_unit, "M" = 1e6, "K" = 1e3, "B" = 1e9, "1" = 1)
    ...
    if (input$mechanisms == "amc") {
      ...
      milestone_stages <- df %>% filter(`Milestone?` == TRUE)
      if (nrow(milestone_stages) == 0) return(NULL)
    ...
    if (is.null(topup) || is.na(topup) || is.infinite(topup) ||
        is.null(target_amount_val) || is.na(target_amount_val) || is.infinite(target_amount_val) ||
        is.null(revenue_start_month) || is.na(revenue_start_month) || is.infinite(revenue_start_month)) {
      return(NULL)
    }
    ...
    if (any(is.na(ac$cumulative_payment_pv)) || any(is.infinite(ac$cumulative_payment_pv))) {
      return(list(
        feasible = FALSE,
        min_topup = NA,
        current_topup = topup,
    ...
    if (!feasible) {
      ...
      if (is.na(sum_discounted_units) || sum_discounted_units <= 0) {
        min_topup <- NA
      } else {
  ...
  output$amc_feasibility_warning <- renderUI({
    ...
    if (!check$feasible) {
      div(
        ...
        p(style = "margin: 0; color: #991b1b; font-size: 13px; font-weight: 600; line-height: 1.5;",
          sprintf("⚠️ Current top-up insufficient. Minimum needed: $%s/unit", 
                  format(round(check$min_topup, 2), big.mark = ",", scientific = FALSE))),
        p(style = "margin: 8px 0 0 0; color: #7f1d1d; font-size: 12px; font-style: italic",
  ...
  amc_units_validation <- reactive({
    ...
    if (is.null(input$amc_calculation_mode) || input$amc_calculation_mode != "units_to_topup") {
      return(NULL)
    }
    ...
    if (is.null(input$amc_target_units) || is.na(input$amc_target_units) || input$amc_target_units <= 0) {
      return(NULL)
    }
    ...
    if (is.na(total_units_available) || total_units_available <= 0) {
      return(NULL)
    }
    ...
    if (input$amc_target_units > total_units_available) {
      return(list(
        ...
        error = sprintf("Target units (%s) exceed lifecycle capacity (%s units)",
                        format(round(input$amc_target_units, 0), big.mark = ",", scientific = FALSE),
                        format(round(total_units_available, 0), big.mark = ",",scientific = FALSE))
      ))
    }
  ...
  output$stage_instructions <- renderUI({
    ...
    if (!is.null(input$mechanisms) && 'milestones_amc' == input$mechanisms) {
    ...
    div(class = "instruction-text",
        HTML(paste0(base_instructions, milestone_instructions))
    )
  })
  ...
  DF <- reactive({
    ...
    if (!is.null(input$stages_table)) {
      ...
      if ("Probability" %in% colnames(current_data) && !("Probability (%)" %in% colnames(current_data))) {
        current_data$`Probability (%)` <- current_data$Probability * 100
        current_data$Probability <- NULL
      }
      if ("Stage Share" %in% colnames(current_data) && !("Stage Share (%)" %in% colnames(current_data))) {
        current_data$`Stage Share (%)` <- current_data$`Stage Share` * 100
        current_data$`Stage Share` <- NULL
      }
      
      # Ensure columns are in the right order
      col_order <- c("Stage", "Stage Name", "Duration", "Cost", "Probability (%)", 
                     "Share of costs covered by other sources (%)", "Milestone?", "Stage Share (%)")
  ...
  output$stages_table <- renderRHandsontable({
    ...
    show_milestones <- !is.null(input$mechanisms) && 'milestones_amc' %in% input$mechanisms
    
    if (show_milestones) {
      rht <- rhandsontable(df, rowHeaders = NULL, stretchH = "all", width = "100%", height = "auto") %>%
      ...
      rht <- rhandsontable(df_subset, rowHeaders = NULL, stretchH = "all", width = "100%", height = "auto") %>%
        hot_col(1, readOnly = TRUE, width = 50) %>%  # Stage column
        hot_col(2, width = 120) %>%  # Stage Name column
        hot_col(3, width = 80, format = "0.0") %>%  # Duration column
  ...
  observe({
    ...
    if (is.null(df) || nrow(df) == 0 || !("Stage Name" %in% names(df))) {
      return()
    }
    ...
    updateSelectInput(session, "revenue_start_stage",
  ...
  observe({
    ...
    if (!is.null(input$model_revenue) && input$model_revenue && !is.null(input$revenue_start_stage)) {
      default_selection <- input$revenue_start_stage
    }
    
    updateSelectInput(session, "amc_payout_start_stage",
                      choices = stage_choices,
                      selected = default_selection)
  }) %>% bindEvent(input$stages_table, input$model_revenue, input$revenue_start_stage)
    ...
    showModal(modalDialog(
      title = div(
        ...
        span("Probability vs Prize Cost", 
             style = "font-size: 1.5rem; font-weight: 700; color: #662260;"),
  ...
  calculate_amc_for_n_v2 <- function(n, prob_of_success, net_cost_to_attempt, discount_rate, 
                                     topup, ac_template, unit_factor, revenue_start_month,
                                     calc_mode = "topup_to_units", target_units = NULL) {
    ...
    if (calc_mode == "topup_to_units" && !is.null(topup) && topup > 0) {
    ...
    } else if (calc_mode == "units_to_topup" && !is.null(target_units) && target_units > 0) {
      # Call the same function the purple card uses
      topup_result <- calculate_topup_from_units(
        target_units = target_units,
        ac_template = ac_template,
  ...
  generate_prize_plot <- function() {
    ...
    max_achievable_prob <- if (use_feasibility) eta_prop else 1
      ...
      for (name in input$selected_archetypes) {
        ...
        if (!is.null(arch)) {
          ...
          if (!is.null(arch_res$prob_of_success) && 
              ...
              (!use_feasibility || abs(arch_res$eta - eta) < 0.0001)) {
    ...
    if (mech == "shared") {
            ...
            layout(
              ...
              annotations = list(
                list(
                  ...
                  yref = "paper",
                  showarrow = FALSE,
                  font = list(size = 16, color = "#662260", family = "Proxima Nova")
                ),
                list(
                  x = 0.5,
                  y = 0.15,
      ...
      ac_template <- adoption_curve()
      unit_factor <- switch(results$cost_unit, "M" = 1e6, "K" = 1e3, "B" = 1e9, "1" = 1)
      
      # Get the calculation mode and parameters
      calc_mode <- if (!is.null(input$amc_calculation_mode)) input$amc_calculation_mode else "topup_to_units"
      ...
      if (calc_mode == "units_to_topup") {
            ...
            layout(
              ...
              annotations = list(
                list(
                  ...
                  font = list(size = 20, color = "#662260", family = "Proxima Nova", weight = "bold")
                ),
                list(
                  x = 0.5, y = 0.3,
                  text = paste0(
                  ...
                  xref = "paper", yref = "paper", showarrow = FALSE,
                  font = list(size = 14, color = "#666", family = "Proxima Nova")
                ),
                list(
                  x = 0.5, y = 0.15, text = "📊", 
                  xref = "paper", yref = "paper", 
                  showarrow = FALSE, font = list(size = 48)
                )
              ),
              plot_bgcolor = "rgba(242, 215, 238, 0.1)",
              paper_bgcolor = "#ffffff",
      ...
      } else if (calc_mode == "topup_to_units" && !is.null(results$amc$topup) && results$amc$topup > 0){
        ...
        plot_data <- data.frame(n = n_values) %>%
          dplyr::rowwise() %>%
          dplyr::mutate(
            amc_calc = list(calculate_amc_for_n_v2(
              n, prob_of_success, results$net_cost_to_attempt,
            ...
            plot_ly() %>%
              layout(
                ...
                annotations = list(
                  ...
                  list(x = 0.5, y = 0.35, text = "📊", xref = "paper", yref = "paper", 
                       showarrow = FALSE, font = list(size = 48))
                ),
                plot_bgcolor = "rgba(242, 215, 238, 0.1)",
                paper_bgcolor = "#ffffff",
          ...
          plot_ly() %>%
            layout(
              ...
              annotations = list(
                ...
                list(x = 0.5, y = 0.35, text = "⚙️", xref = "paper", yref = "paper", 
                     showarrow = FALSE, font = list(size = 48))
              ),
              plot_bgcolor = "rgba(242, 215, 238, 0.1)",
              paper_bgcolor = "#ffffff",
      ...
      plot_data <- data.frame(n = n_values) %>%
        ...
        dplyr::mutate(
          base_global_pos = 1 - (1 - prob_of_success)^n,
          global_pos = if (use_feasibility) pmin(base_global_pos * eta, eta) else base_global_pos, # CHANGED THIS 
          gross_prize = expected_shared_prize(n, prob_of_success, cost_to_attempt),
          net_prize = max(0, gross_prize - revenue_pv),  # Subtract revenue here
      ...
      if (topup > 0 && !is.null(ac_template) && nrow(ac_template) > 0) {
        plot_data <- plot_data %>%
          dplyr::rowwise() %>%
          dplyr::mutate(
            amc_calc = list(calculate_amc_for_target(
            ...
            amc_amount = ifelse(amc_calc$covers_prize, amc_calc$amc_size_undiscounted, NA),
            x_value = ifelse(amc_calc$covers_prize, milestone_payment + amc_calc$amc_size_undiscounted, NA)
          ) %>%
          dplyr::ungroup() %>%
          dplyr::select(-amc_calc) %>%
    ...
    # Add current curve
    if (show_current_curve && !is.null(plot_data) && nrow(plot_data) > 0) {
      p <- p %>% add_trace(
        data = plot_data,
        x = ~x_value,
        y = ~global_pos,
        ...
        hoverinfo = "text",
        text = ~paste0(
          "Current<br>Attempts: ", n,
          "<br>Global PoS: ", sprintf("%.1f%%", global_pos * 100),
          "<br>Cost: $", format(round(x_value, 1), big.mark = ","), substr(unit_label, 2, 2)
        ),
        name = "Current"
      )
    ...
    if (!is.null(input$selected_archetypes) && length(input$selected_archetypes) > 0) {
      colors <- c("#662260", "#58595B", "#FFB52C", "#DE7C5A", "#8DB790", "#F2D7EE", "#D3D4D9")
      ...
      for (i in seq_along(selected_archetypes)) {
        ...
        arch <- saved_archetypes$data[[name]]
        if (is.null(arch)) next
        
        arch_res <- arch$results
        if (is.null(arch_res$prob_of_success) || is.null(arch_res$cost_to_attempt)) next
        ...
        arch_max_achievable <- if (arch_use_feas) arch_eta_prop else 1
        ...
        arch_data <- data.frame(n = arch_n_values) %>%
          ...
          dplyr::mutate(
            ...
            global_pos = if (arch_use_feas) pmin(base_global_pos * arch_eta_prop, arch_eta_prop) else base_global_pos,
        ...
        line_style <- if (is_current_display) {
          ...
          list(color = arch_color, width = 2, dash = "dot")
        }
        
        mode_style <- if (is_current_display) "lines+markers" else "lines"
        marker_style <- if (is_current_display) list(color = arch_color, size = 7) else list()
        
        arch_data <- arch_data %>%
          dplyr::mutate(
            hover_text = paste0(
              display_name, if (is_current_display) " (current)" else "",
              "<br>Attempts: ", n,
              "<br>Global PoS: ", sprintf("%.1f%%", global_pos * 100),
              "<br>Cost (prize equiv.): $", format(round(x_value, 1), big.mark = ","), substr(unit_label, 2, 2)
            )
          )
        ...
        p <- p %>% add_trace(
          ...
          hoverinfo = "text",
          text = ~hover_text,
          name = if (is_current_display) paste(display_name, "(current)") else display_name,
          showlegend = TRUE
        )
        ...
        if (nrow(arch_point) == 1) {
          p <- p %>% add_markers(
            ...
            y = ~global_pos,
            marker = list(
              color = if (is_current_display) "#FFFFFF" else arch_color,
              size = if (is_current_display) 14 else 10,
              symbol = if (is_current_display) "circle" else "diamond",
              line = list(color = if (is_current_display) arch_color else arch_color,
                          width = if (is_current_display) 3 else 0)
            ),
            name = paste(display_name, "selection"),
            hoverinfo = "text",
            text = paste0(
              display_name, if (is_current_display) " (current)" else "", " – selection<br>",
              "Firms: ", arch_res$n_firms,
              "<br>Global PoS: ", sprintf("%.1f%%", arch_point$global_pos * 100),
              "<br>Cost (prize equiv.): $", format(round(arch_point$x_value[1], 1), big.mark = ","), substr(unit_label, 2, 2)
            ),
            showlegend = FALSE
          )
        }
    ...
    p %>%
      layout(
        xaxis = list(
          title = list(text = paste0("Size of pull mechanism, ", unit_label), standoff = 20,
                       font = list(family="Proxima Nova", size=14, color="#58595B")),
          gridcolor = "#D3D4D9",
          tickformat = ",.0f"
        ),
        yaxis = list(
          title = list(text = "Probability at least one firm succeeds", standoff = 15,
                       font = list(family="Proxima Nova", size=14, color="#58595B")),
          gridcolor = "#D3D4D9",
          tickformat = ".0%",
          range = c(0, y_max)
        ...
        plot_bgcolor = "#ffffff",
        paper_bgcolor = "#ffffff",
        font = list(family = "Proxima Nova, sans-serif", size = 12, color = "#58595B"),
        hovermode = "closest",
        showlegend = TRUE,
        ...
        margin = list(t = 30, r = 40, b = 130, l = 60),
        autosize = TRUE
      ) %>%
  ...
  output$results <- renderUI({
    ...
    incentive_display <- div(
      id = "incentive_summary",  # ADD THIS ID
      # Section Header
      div(
      ...
      if(results$use_feasibility) {
        div(
          ...
          p(style = "margin: 0; font-size: 13px; color: #58595B; font-weight: 500;",
            HTML(sprintf("⚙️ <strong>Advanced settings enabled:</strong> Global possibility (η = %.0f%%)", results$eta ))))
      },
         ...
         cursor: default;",
            ...
            div(
              ...
              if (abs(results$global_pos * 100 - results$target_probability) > 0.001) {
                div(sprintf('(Target: %.1f%%)', results$target_probability),
          ...
          if (!is.null(results$mechanisms) && 'shared' %in% results$mechanisms) {
            div(
              style = "padding: 24px;
                 border-left: 4px solid #662260;
                 background-color: rgba(255,255,255,0.5);
              ...
              div("Prize",
                  style = "font-size: 11px; color: #662260; text-transform: uppercase; letter-spacing: 1px;
                    margin-bottom: 12px; font-weight: 600;"),
              
              # Show post-revenue amount (net) as main number if modeling revenue
              div(
                if (!is.null(results$model_revenue) && results$model_revenue && results$revenue_pv > 0) {
                  sprintf("$%s%s", format(round(max(0, results$shared_prize - results$revenue_pv), 1), big.mark=","), substr(unit_label, 2, 2))
                } else {
          ...
          if (!is.null(results$mechanisms) && 'amc' %in% results$mechanisms) {
            amc <- results$amc
            has_error <- !is.null(amc) && (!is.na(amc$covers_prize) && !amc$covers_prize)
            
            # FOR UNITS_TO_TOPUP MODE: Calculate the exact value shown in scenarios table
            displayed_amc_value <- NULL
            displayed_topup <- NULL  # ← ADD THIS LINE
            if (!is.null(input$amc_calculation_mode) && 
                input$amc_calculation_mode == "units_to_topup" &&
                !is.null(input$amc_target_units) && !is.na(input$amc_target_units)) {
              ...
              topup_result <- calculate_topup_from_units(
                target_units = input$amc_target_units,
                ac_template = ac_template,
                discount_rate = input$discount_rate,  # NOT divided by 100
                unit_factor = unit_factor,
                target_amount = target_amount,
                # I changed the 12 out of the below once....
                revenue_start_month = if (!is.null(results$revenue_start_month)) results$revenue_start_month  else 0  # Convert years to months
              )
              
              if (topup_result$feasible) {
                displayed_amc_value <- (topup_result$topup * topup_result$units_covered) / unit_factor
                displayed_topup <- topup_result$topup  # ← ADD THIS LINE
              }
            }
            
            div(
              style = paste0("padding: 24px;
               border-left: 4px solid ", if(has_error) "rgba(239, 68, 68, 0.8)" else "#662260", ";
               background-color: ", if(has_error) "rgba(239, 68, 68, 0.15)" else "rgba(255,255,255,0.5)", ";
               backdrop-filter: blur(10px);
               border-radius: 12px;
               display: flex; flex-direction: column;
               transition: all 0.3s ease;
                ...
                div(
                  div(
                    if(has_error) {
                      ...
                      sprintf("$%s%s", 
                              format(round(displayed_amc_value, 1), big.mark = ",", scientific = FALSE),
                              substr(unit_label, 2, 2))
                    } else {
                      # This should match the table: topup * units_covered in the right units
                      sprintf("$%s%s", 
                              if (is.infinite(amc$amc_size_undiscounted)) "∞" 
                              else format(round(amc$amc_size_undiscounted, 1), big.mark = ",", scientific = FALSE),
                              substr(unit_label, 2, 2))
                    },
                    style = "font-size: 32px; font-weight: 800; color: #662260;"),
                  ...
                  if (!has_error && 
                      ...
                      !is.na(amc$amc_size_pv) &&  # ADD THIS
                      !is.infinite(amc$amc_size_pv)) {
                    div(
                      sprintf("(PV at launch: $%s%s)", format(round(amc$amc_size_pv, 1), big.mark=","), substr(unit_label, 2, 2)),
                  ...
                  div(
                    if(has_error) {
                      ...
                      sprintf("Covers %s units", 
                              if(is.infinite(amc$units_to_cover_total)) "∞" else format(round(amc$units_to_cover_total, 0), big.mark = ",", scientific = FALSE))
                    },
                  ...
                  if(!is.null(amc$topup) && amc$topup > 0) {
                    is_calculated <- !is.null(amc$optimization_mode) && amc$optimization_mode == "units_to_topup"
                    topup_to_show <- if(!is.null(displayed_topup)) displayed_topup else amc$topup  # ← USE DISPLAYED VALUE
                    div(
                      sprintf("Top-up: $%s/unit%s", 
                              format(round(topup_to_show, 1), big.mark = ","),
                              if(is_calculated) " (calculated)" else ""),
          ...
          if (!is.null(results$mechanisms) && 'milestones_amc' %in% results$mechanisms) {
            ms_data <- results$milestones
            has_error <- is.null(ms_data) || !ms_data$valid
            
            # Check if AMC has an error
            has_amc_error <- !has_error && !is.null(ms_data$amc_component) && 
            ...
            if (!is.null(input$amc_calculation_mode) && 
              ...
              topup_result <- calculate_topup_from_units(
                ...
                unit_factor = unit_factor,
                target_amount = target_amount,
                revenue_start_month = if (!is.null(results$revenue_start_month)) results$revenue_start_month else 0
              )
              
              if (topup_result$feasible) {
                displayed_milestone_amc_value <- (topup_result$topup * topup_result$units_covered) / unit_factor
                displayed_milestone_topup <- topup_result$topup  # ← ADD THIS
              }
            }
            
            div(
              style = paste0("padding: 24px;
                 border-left: 4px solid ", if(has_error || has_amc_error) "rgba(239, 68, 68, 0.8)" else "#662260", ";
                 background-color: ", if(has_error || has_amc_error) "rgba(239, 68, 68, 0.5)" else "rgba(255,255,255,0.5)", ";
                 backdrop-filter: blur(10px);
                 border-radius: 12px;
                 display: flex; flex-direction: column;
                 transition: all 0.3s ease;
              ...
              if (!is.null(ms_data) && !has_error) {
                div(
                  ...
                  if (!has_amc_error) {
                    ...
                    div(
                      sprintf("Milestones: $%s%s | AMC: $%s%s", 
                              format(round(ms_data$total_milestone_payments, 1), big.mark = ","),
                              substr(unit_label, 2, 2),
      ...
      if (!has_amc_error) {
        ...
        div(
          sprintf("%d milestone%s (%.0f%% of total)", 
                  ms_data$n_milestones,
                  if(ms_data$n_milestones != 1) "s" else "",
          ...
          style = "font-size: 12px; font-weight: 500; color: #662260;margin-top: 4px; font-style: italic;"
                ...
                div(
                  div(if(has_error) "Invalid configuration" else "Milestones required", 
    ...
    if (!is.null(results$commercially_viable) && results$commercially_viable) {
    ...
    amc_warning <- NULL
    if (!is.null(results$mechanisms) && results$mechanisms == 'amc') {
      if (!is.null(results$amc) && !results$amc$covers_prize) {
        amc_warning <- div(
          class = "warning-box",
          style = "margin-top: 20px; margin-bottom: 20px;",
          p(class = "warning-text", 
            HTML("⚠️ <strong>AMC Warning:</strong> The AMC will never cover the prize cost with the current parameters. The cumulative subsidy payments over the product lifecycle do not reach the prize amount. Consider increasing the top-up amount, lifecycle duration, or uptake rate."))
        )
      }
    }
    ...
    if (!is.null(results$mechanisms) && 
        (results$mechanisms == 'amc' || results$mechanisms == 'milestones_amc') &&
      ...
      if (!is.null(user_units) && !is.na(user_units) && user_units > 0) {
      ...
      scenarios <- lapply(percentages, function(pct) {
        ...
        if (result$feasible) {
          list(
            ...
            feasible = TRUE,
            is_custom = !is.null(user_units) && !is.na(user_units) && 
      ...
      scenarios <- Filter(function(s) s$feasible, scenarios)
      ...
      if (!is.null(scenarios) && length(scenarios) > 0) {
        amc_scenarios_display <- div(
          ...
          div(
            ...
            p(style = "color: #58595B; font-size: 13px; margin-bottom: 20px; font-style: italic;",
              sprintf("Different coverage levels and their required subsidy amounts (total lifecycle capacity: %s units):",
                      format(round(total_units_available, 0), big.mark = ",", scientific = FALSE)))
          ),
          ...
          div(
            ...
            lapply(seq_along(scenarios), function(i) {
              ...
              if (!is.null(user_units) && !is.na(user_units) && !is.null(s$units_covered) && !is.na(s$units_covered)) {
              ...
              div(
                style = paste0(
                  ...
                  if (is_current) "background: rgba(102, 34, 96, 0.08); border-left: 4px solid #662260; font-weight: 600;" 
                  else if (is_custom) "background: rgba(251, 191, 36, 0.08); border-left: 4px solid #f59e0b;" 
                  else if (i %% 2 == 0) "background: rgba(0,0,0,0.02);" 
                  else ""
                ),
                onmouseover = if (!is_current) "this.style.background='rgba(102, 34, 96, 0.05)';" else "",
                onmouseout = if (!is_current) {
                  if (is_custom) "this.style.background='rgba(251, 191, 36, 0.08)';"
                  else if (i %% 2 == 0) "this.style.background='rgba(0,0,0,0.02)';" 
                ...
                div(
                  ...
                  format(round(s$units_covered, 0), big.mark = ",", scientific = FALSE)
                ),
                div(
                  style = "text-align: right; color: #2d3748; font-weight: 600;",
                  sprintf("$%s", format(round(s$topup, 1), big.mark = ",", scientific = FALSE))
                ),
                div(
                  style = "text-align: right; color: #2d3748; font-weight: 700;",
                  sprintf("$%s%s", 
                          format(round(amc_total_displayed, 1), big.mark = ",", scientific = FALSE),
                          substr(unit_label, 2, 2))
                )
              )
            })
          ...
          div(
            ...
            p(style = "margin: 0; font-size: 12px; color: #58595B; font-style: italic;",
        ...
        if (!is.null(results$amc) && !is.null(results$amc$error)) {
          amc_scenarios_display <- div(
            ...
            p(style = "color: #991b1b; margin: 0; font-weight: 600;",
              "⚠️ Cannot calculate scenarios: ", results$amc$error)
          )
        }
      }
    }
    ...
    if (!is.null(results$mechanisms) && results$mechanisms == 'milestones_amc') {
      if (!is.null(results$milestones)) {
        if (!results$milestones$valid) {
          milestones_warning <- div(
            class = "warning-box",
            style = "margin-top: 20px; margin-bottom: 20px;",
            p(class = "warning-text", 
              HTML(sprintf("⚠️ <strong>Milestones + AMC Warning:</strong> %s", 
          ...
          milestones_warning <- div(
            ...
            style = "margin-top: 20px; margin-bottom: 20px;",
            p(class = "warning-text", 
              HTML("⚠️ <strong>Milestones + AMC Warning:</strong> The AMC component will never cover its target amount with the current parameters. The cumulative subsidy payments over the product lifecycle do not reach the required AMC portion. Consider increasing the top-up amount, lifecycle duration, or uptake rate."))
          )
        }
      }
    }
    ...
    if (!is.null(results$milestones) && results$milestones$valid) {
      ...
      milestone_display <- div(
        ...
        # Section Header
        div(
          h4("Milestone Payment Structure",
        ...
        div(
          ...
          div(
            ...
            lapply(ms$milestones, function(m) {
              div(
                ...
                div(style = "display: flex; justify-content: space-between; align-items: center; color: white",
                    ...
                    # Left: Stage info
                    div(
                      div(sprintf("Stage %d: %s", m$stage, m$stage_name),
                          style = "font-size: 16px; font-weight: 700; margin-bottom: 5px; color: white"),
                      div(sprintf("%.1f%% of total prize | Prob to stage: %.1f%%",
                                  m$stage_pct, m$prob_to_stage * 100),
            ...
            if (!is.null(ms$amc_component) && ms$amc_component$amc_pct > 0) {
              amc_comp <- ms$amc_component
              has_amc_error <- !is.na(amc_comp$amc_covers_target) && !amc_comp$amc_covers_target
              
              div(
                style = paste0("background-color: ", 
                               if(has_amc_error) "rgba(239, 68, 68, 0.2)" else "rgba(255,255,255,0.15)",
                               ";
       padding: 15px 20px;
       border-radius: 10px;
       border-left: 4px solid ", 
                               if(has_amc_error) "rgba(239, 68, 68, 0.8)" else "white",
                               ";"),
                ...
                div(style = "display: flex; justify-content: space-between; align-items: center; color: white",
                    ...
                    # Left: AMC info
                    div(
                      div(sprintf("After Stage %d: %s - AMC", amc_comp$final_stage, amc_comp$final_stage_name),
                          style = "font-size: 16px; font-weight: 700; margin-bottom: 5px; color: white"),
                      div(
                        if (!is.na(amc_comp$amc_covers_target) && !has_amc_error) {
                          ...
                          sprintf("%.1f%% of total prize | Covers %s units | $%s/unit top-up",
                                  amc_comp$amc_pct,
                                  format(round(units_to_show, 0), big.mark = ",", scientific = FALSE),
                                  format(round(topup_to_show, 2), big.mark = ",", scientific = FALSE))
                    ...
                    div(
                      if (has_amc_error) {
                        ...
                        amc_amount <- if (!is.null(displayed_milestone_amc_value)) {
                          displayed_milestone_amc_value
                        } else if (!is.na(amc_comp$amc_covers_target) && amc_comp$amc_covers_target) {
     ...
     display: flex; justify-content: space-between; align-items: center;",
            ...
            div(style = "color: white; font-size: 14px; font-weight: 500;",
                sprintf("%d firms incentivized | %.1f%% global probability of success",
                        ms$n_firms, ms$global_pos * 100)),
            
            div(
              div("Total Incentive",
                  style = "font-size: 12px; text-align: right; color: white"),
              div({
    ...
    if (!is.null(input$model_revenue) && input$model_revenue && !is.null(results$revenue_pv) && results$revenue_pv > 0) {
      ...
      revenue_info_display <- div(
        ...
        font-size: 1.7rem; font-weight: 800;"),
          ...
          p(style = "color: #065f46; font-size: 13px; margin-bottom: 15px; font-weight: 600; font-style: italic;",
            if (!is.null(results$commercially_viable) && results$commercially_viable) {
              "✓ Project is commercially viable! Expected revenue covers all development costs."
            } else {
              revenue_start_msg <- if (!is.null(results$revenue_start_stage) && results$revenue_start_stage > 0) {
                df <- results$table_data
                stage_name <- df$Name[results$revenue_start_stage]
                sprintf("Revenue begins after completing %s (at month %.0f). Expected revenue (probability-adjusted) reduces the net cost.",
                        stage_name,
        ...
        div(
         ...
         margin-bottom: 20px;",
          ...
          div(style = "color: white; font-size: 15px; font-weight: 600; margin-bottom: 15px;",
              "How Revenue Reduces Required Incentive"),
          
          div(style = "display: grid; grid-template-columns: 1fr auto 1fr auto 1fr; gap: 15px; align-items: center; justify-content: center;",
              ...
              div(
                style = "text-align: center; padding: 15px;
                 ...
                 border-radius: 10px;",
                div("Expected Development Costs", 
              ...
              div(
                ...
                div(style = "font-size: 10px; color: rgba(255,255,255,0.8); margin-bottom: 6px; font-style: italic;",
                    sprintf("(%.0f%% prob × $%s%s PV)", 
          ...
          div(style = "margin-top: 15px; padding-top: 15px; border-top: 1px solid rgba(255,255,255,0.3);
              ...
              if (!is.null(results$commercially_viable) && results$commercially_viable) {
                sprintf("Expected revenue ($%s%s) fully covers expected development costs ($%s%s). No public incentive is needed.",
                        format(round(results$revenue_pv, 1), big.mark=","),
                        substr(unit_label, 2, 2),
                        format(round(results$cost_to_attempt, 1), big.mark=","),
                        substr(unit_label, 2, 2))
              } else {
                sprintf("Expected revenue of $%s%s (probability-adjusted) reduces net cost to $%s%s. The incentive mechanism is sized to cover this net cost.",
                        format(round(results$revenue_pv, 1), big.mark=","),
                        substr(unit_label, 2, 2),
                        format(round(results$net_cost_to_attempt, 1), big.mark=","),
                        substr(unit_label, 2, 2))
    ...
    save_section <- div(
      ...
      div(style = "background: rgba(102, 34, 96, 0.05); 
               padding: 24px; border-radius: 16px; border: 2px solid rgba(102, 34, 96, 0.1);",
          p(style = "color: #58595B; font-size: 13px; margin-bottom: 15px; line-height: 1.5;",
          ...
          actionButton("save_archetype", "Save Current Results", class = "btn-block orange-button", style = "background: #FFB52C; margin-top: 15px; color: black;  box-shadow: 0 4px 20px #FFB52C;")
      )
    )
    ...
    if (!is.null(input$selected_archetypes) && length(input$selected_archetypes) > 0) {
        ...
        if (!is.null(arch)) {
          ...
          div(
            style = paste0("flex: 1; min-width: 240px; margin: 10px; padding: 24px; 
                     ...
                     box-shadow: 0 4px 20px rgba(0,0,0,0.08);
                     transition: all 0.3s ease;"),
            h5(name, style = paste0("color: ", colors[color_idx], "; margin-bottom: 16px; margin-top: 0; font-size: 1.2rem; font-weight: 800;")),
            tags$table(style = "width: 100%; font-size: 14px; font-family: 'Proxima Nova', sans-serif;",
                       tags$tr(
                       ...
                       tags$tr(
                         ...
                         tags$td(sprintf("%.1f%%", arch$results$prob_of_success*100 ), 
                                 style = "text-align: right; padding: 8px 4px; font-weight: 700; color: #2d3748;")
                       ),
                       tags$tr(
                       ...
                       tags$tr(
                         ...
                         tags$td(sprintf("%.1f%%", arch$results$global_pos*100 ), 
                                 style = "text-align: right; padding: 8px 4px; font-weight: 700; color: #2d3748;")
                       ),
                       tags$tr(
      ...
      comparison_display <- div(
        hr(style = "border: none; height: 2px; background: #662260; margin: 30px 0;"),
        div(
          h4("Saved Archetypes", style = "background: #662260;
                   background-clip: text;
                   -webkit-background-clip: text;
                   -webkit-text-fill-color: transparent;
      ...
      div(class = "results-container",
          incentive_display,
          amc_warning, 
          amc_scenarios_display,  # ADD THIS LINE
          milestones_warning,
          milestone_display,
          revenue_info_display, 
          individual_firm_display,
  ...
  output$plot_section <- renderUI({
    ...
    tagList(
      div(style = "display: flex; justify-content: space-between; align-items: center; margin-bottom: 10px;",
          h4("Probability vs Prize Cost",
             style = "font-size: 1.3rem; margin-bottom: 0; color: #662260; font-weight: 600;"),
      ...
      div(
        p(style = "color: #58595B; font-size: 13px; margin-bottom: 12px; font-style: italic;",
          "Select saved scenarios below to compare them on the chart:"),
        selectInput("selected_archetypes", "Select Scenarios to Display:",
